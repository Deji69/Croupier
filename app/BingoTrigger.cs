...
namespace Croupier {
	...
	public class BingoTriggerEnum<TEnum> : BingoTriggerExpression where TEnum : struct, Enum {
		...
		public void Load(JsonElement json, string propName, Func<string, TEnum>? converter = null) {
			...
			values = prop.ValueKind switch {
				...
				JsonValueKind.Number => [Enum.Parse<TEnum>(prop.GetInt64().ToString())],
		...
		public static List<TEnum> LoadEnumArray(JsonElement prop, Func<string, TEnum> converter) {
			...
			foreach (var item in prop.EnumerateArray()) {
				if (item.ValueKind == JsonValueKind.String)
					...
					coll.Add(Enum.Parse<TEnum>(prop.GetInt64().ToString()));
				else
	...
	public class BingoTriggerPosition : BingoTriggerExpression {
		...
		public void Load(JsonElement json, string propName) {
			...
			if (prop.ValueKind == JsonValueKind.Object)
				...
				throw new BingoTileConfigException("Property 'Position' should be an object, array or string consisting of 3 comma-separated numbers.");
		}
		...
		private void LoadString(JsonElement json) {
			var str = json.GetString() ?? throw new BingoTileConfigException("Invalid string for 'Position'.");
			var parts = str.Split(',');
			if (parts.Length != 3)
				throw new BingoTileConfigException("String for 'Position' must contain 3 comma-separated numbers.");
			if (!double.TryParse(parts[0], out var x))
				throw new BingoTileConfigException("Failed to parse X coordinate in 'Position'.");
			if (!double.TryParse(parts[1], out var y))
				throw new BingoTileConfigException("Failed to parse Y coordinate in 'Position'.");
			if (!double.TryParse(parts[2], out var z))
				throw new BingoTileConfigException("Failed to parse Z coordinate in 'Position'.");
			X = x;
			Y = y;
			Z = z;
		}
	...
	public class BingoTrigger : IBingoTrigger {
		...
		public virtual bool HasCount() {
			return Count.HasValue || (randomCountMax.HasValue && randomCountMin.HasValue) || randomCountVals.Count > 0;
		}
		...
		private static Func<JsonHolder, BingoTrigger>? FromJson_TriggerProp(string propName) {
			return propName switch {
				...
				"AgilityEnter" => jh => new BingoTriggerAgilityEnter(jh.json),
				...
				"ChangeDisguise" => jh => new BingoTriggerChangeDisguise(jh.json),
				...
				"OnEntered" => jh => new BingoTriggerOnEntered(jh.json),
				"OnEvacuationStarted" => jh => new BingoTriggerOnEvacuationStarted(jh.json),
				"OnHitByImpulse" => jh => new BingoTriggerOnHitByImpulse(jh.json),
				"OnInitialFracture" => jh => new BingoTriggerOnInitialFracture(jh.json),
				"OnInteractionItemUsed" => jh => new BingoTriggerOnInteractionItemUsed(jh.json),
				"OnInterrupted" => jh => new BingoTriggerOnInterrupted(jh.json),
				...
				"OnLeaving" => jh => new BingoTriggerOnLeaving(jh.json),
				"OnOut" => jh => new BingoTriggerOnOut(jh.json),
				"OnPickup" => jh => new BingoTriggerOnPickup(jh.json),
				"OnPutInContainer" => jh => new BingoTriggerOnPutInContainer(jh.json),
				"OnSabotageSetpiece" => jh => new BingoTriggerOnSabotageSetpiece(jh.json),
				"OnShot" => jh => new BingoTriggerOnShot(jh.json),
	...
	public class BingoTriggerItem : IBingoTrigger {
		...
		public BingoTriggerItem(JsonElement json) {
			if (json.ValueKind != JsonValueKind.Object) return;
	...
	public class BingoTriggerLocation : IBingoTrigger {
		...
		public BingoTriggerLocation(JsonElement json) : base() {
			if (json.ValueKind != JsonValueKind.Object) return;
		...
		public bool Test(EventValue ev, BingoTileState state) {
			return ev is LocationEventValue v
	...
	public class BingoTriggerPlayer : IBingoTrigger {
		readonly BingoTriggerString HeroOutfit = new();
		readonly BingoTriggerBool HeroOutfitIsHitmanSuit = new();
		readonly BingoTriggerString Outfit = new();
		readonly BingoTriggerBool OutfitIsHitmanSuit = new();

		public BingoTriggerPlayer(JsonElement json) : base() {
			if (json.ValueKind != JsonValueKind.Object) return;
			HeroOutfit.Load(json, nameof(HeroOutfit));
			HeroOutfitIsHitmanSuit.Load(json, nameof(HeroOutfitIsHitmanSuit));
			Outfit.Load(json, nameof(Outfit));
			OutfitIsHitmanSuit.Load(json, nameof(OutfitIsHitmanSuit));
		}

		public bool Test(EventValue ev, BingoTileState state) {
			return ev is PlayerEventValue v
				&& HeroOutfit.Test(v.HeroOutfit, state)
				&& HeroOutfitIsHitmanSuit.Test(v.HeroOutfitIsHitmanSuit, state)
				&& Outfit.Test(v.Outfit, state)
				&& OutfitIsHitmanSuit.Test(v.OutfitIsHitmanSuit, state);
		}
	}

	public class BingoTriggerComplication : BingoTrigger {
		public readonly BingoTrigger Trigger;

		public BingoTriggerComplication(BingoTrigger trigger) : base() {
			Trigger = trigger;
			Count = trigger.Count;
		}
	...
	public class BingoTriggerActorSick : BingoTrigger {
		...
		public BingoTriggerActorSick(JsonElement json) : base(json) {
			...
			ActorName.Load(json, nameof(ActorName));
			ActorRepositoryId.Load(json, nameof(ActorRepositoryId));
			ActorType.Load(json, nameof(ActorType), str => str switch {
				"Guard" => EActorType.eAT_Guard,
				"Civilian" => EActorType.eAT_Civilian,
				_ => throw new BingoConfigException("Invalid ActorType - expected 'Guard' or 'Civilian'.")
			});
			...
			ItemRepositoryId.Load(json, nameof(ItemRepositoryId));
			SetPieceRepositoryId.Load(json, nameof(SetPieceRepositoryId));
		}
	...
	public class BingoTriggerAgilityEnter(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is AgilityEnterEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
	...
	public class BingoTriggerItemStashed : BingoTrigger {
		...
		readonly BingoTriggerUInt ActorId = new();
		readonly BingoTriggerString ActorName = new();
		readonly BingoTriggerString ItemId = new();
		readonly BingoTriggerString ItemTypeId = new();
		...
		public BingoTriggerItemStashed(JsonElement json) : base(json) {
			...
			ActorId.Load(json, nameof(ActorId));
			ActorName.Load(json, nameof(ActorName));
			ItemId.Load(json, nameof(ItemId));
			ItemTypeId.Load(json, nameof(ItemTypeId));
			RepositoryId.Load(json, nameof(RepositoryId));
			Unique.Load(json, nameof(Unique));
		}

		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is ItemStashedEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
				&& ActorId.Test(v.ActorId, state)
				&& ActorName.Test(v.ActorName, state)
				&& ItemId.Test(v.ItemId, state)
				&& ItemTypeId.Test(v.ItemTypeId, state)
	...
	public class BingoTriggerPlayerShot(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerItem item = new(json);
		readonly BingoTriggerPlayer player = new(json);
	...
	public class BingoTriggerItemThrown(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerCIString RepositoryId = new();
		readonly BingoTriggerItem item = new(json);
		readonly BingoTriggerPlayer player = new(json);
		readonly BingoTriggerLocation location = new(json);
	...
	public class BingoTriggerItemDestroyed : BingoTrigger {
		readonly BingoTriggerCIString RepositoryId = new();
		readonly BingoTriggerItem item;
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerLocation location;
	...
	public class BingoTriggerItemDropped(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerItem item = new(json);
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerPlayer player = new(json);
	...
	public class BingoTriggerProjectileBodyShot(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is ProjectileBodyShotEventValue v
				&& base.Test(v, state)
				&& player.Test(v.Player, state)
				&& location.Test(v.Location, state);
		}
	...
	public class BingoTriggerSecuritySystemRecorder : BingoTrigger {
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerLocation location;
		readonly BingoTriggerEnum<SecuritySystemRecorderEvent> Event = new();
		...
		public BingoTriggerSecuritySystemRecorder(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			Event.Load(json, nameof(Event), s => s switch {
				"CameraDestroyed" => SecuritySystemRecorderEvent.CameraDestroyed,
				"Spotted" => SecuritySystemRecorderEvent.Spotted,
				"Destroyed" => SecuritySystemRecorderEvent.Destroyed,
				"Erased" => SecuritySystemRecorderEvent.Erased,
				_ => SecuritySystemRecorderEvent.Undefined
			});
			Camera.Load(json, nameof(Camera));
			Recorder.Load(json, nameof(Recorder));
			Unique.Load(json, nameof(Unique));
		}
	...
	public class BingoTriggerEnterArea(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerPlayer player = new(json);
	...
	public class BingoTriggerEnterRoom(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerPlayer player = new(json);
	...
	public class BingoTriggerFriskedSuccess(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is FriskedSuccessEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
	...
	public class BingoTriggerChangeDisguise(JsonElement json) : BingoTriggerDisguise(json) {
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is DisguiseEventValue v
				&& ev is not StartingSuitEventValue
				&& base.Test(v, state);
		}
	}
	...
	public class BingoTriggerStartingSuit(JsonElement json) : BingoTriggerDisguise(json) {
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is StartingSuitEventValue v
				&& base.Test(v, state);
		}
	}
	...
	public class BingoTriggerOnIsFullyInCrowd(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnIsFullyInCrowdEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
	...
	public class BingoTriggerOnIsFullyInVegetation(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnIsFullyInVegetationEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
	...
	public class BingoTriggerOnBroken : BingoTrigger {
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerLocation location;
		readonly BingoTriggerPlayer player;

		public BingoTriggerOnBroken(JsonElement json) {
			player = new(json);
			location = new(json);
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnBrokenEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
	...
	public class BingoTriggerOnDestroyed : BingoTrigger {
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerLocation location;
		readonly BingoTriggerPlayer player;

		public BingoTriggerOnDestroyed(JsonElement json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
		}
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnDestroyedEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
				&& EntityID.Test(v.EntityID, state);
	...
	public class BingoTriggerOnEntered : BingoTrigger {
		readonly BingoTriggerLocation location;
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerString RepositoryId = new();
		...
		public BingoTriggerOnEntered(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
			RepositoryId.Load(json, nameof(RepositoryId));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnEnteredEventValue v
				&& base.Test(ev, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnLeaving : BingoTrigger {
		readonly BingoTriggerLocation location;
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerString RepositoryId = new();
		...
		public BingoTriggerOnLeaving(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
			RepositoryId.Load(json, nameof(RepositoryId));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnLeavingEventValue v
				&& base.Test(ev, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnHitByImpulse : BingoTrigger {
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerLocation location;
		readonly BingoTriggerCIString RepositoryId = new();
		readonly BingoTriggerUInt EntityID = new();
		...
		public BingoTriggerOnHitByImpulse(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
			RepositoryId.Load(json, nameof(RepositoryId));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnHitByImpulseEventValue v
				&& base.Test(ev, state)
				&& player.Test(v.Player, state)
				&& location.Test(v.Location, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnInteractionItemUsed : BingoTrigger {
		readonly BingoTriggerPlayer Player;
		readonly BingoTriggerLocation Location;
		readonly BingoTriggerItem Object;
		readonly BingoTriggerItem Item;
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerUnique Unique = new();

		public BingoTriggerOnInteractionItemUsed(JsonElement json) : base(json) {
			Player = new(json);
			Location = new(json);
			Object = json.TryGetProperty("Object", out var obj) ? new(obj) : new(json);
			Item = json.TryGetProperty("Item", out var item) ? new(item) : new(json);
			EntityID.Load(json, nameof(EntityID));
			Unique.Load(json, nameof(Unique));
		}

		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnInteractionItemUsedEventValue v
				&& base.Test(ev, state)
				&& Player.Test(v.Player, state)
				&& Location.Test(v.Location, state)
				&& Item.Test(v.Item, state)
				&& Object.Test(v.Object, state)
				&& EntityID.Test(v.EntityID, state)
				&& Unique.Test(v, state);
		}
	}

	public class BingoTriggerOnInterrupted : BingoTrigger {
		readonly BingoTriggerPlayer Player;
		readonly BingoTriggerLocation Location;
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerUnique Unique = new();

		public BingoTriggerOnInterrupted(JsonElement json) : base(json) {
			Player = new(json);
			Location = new(json);
			EntityID.Load(json, nameof(EntityID));
			Unique.Load(json, nameof(Unique));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnInterruptedEventValue v
				&& base.Test(ev, state)
				&& Player.Test(v.Player, state)
				&& Location.Test(v.Location, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnOut : BingoTrigger {
		readonly BingoTriggerUInt Output = new();
		readonly BingoTriggerUInt EntityID = new();
		readonly BingoTriggerUnique Unique = new();

		public BingoTriggerOnOut(JsonElement json) : base(json) {
			Output.Load(json, nameof(Output));
			EntityID.Load(json, nameof(EntityID));
			Unique.Load(json, nameof(Unique));
		}

		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnOutEventValue v
				&& base.Test(ev, state)
				&& EntityID.Test(v.EntityID, state)
				&& Output.Test(v.Output, state)
				&& Unique.Test(v, state);
		}
	}
	...
	public class BingoTriggerOnPutInContainer(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerPlayer player = new(json);
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerItem item = new(json);

		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnPutInContainerEventValue v
				&& base.Test(ev, state)
				&& player.Test(v.Player, state)
				&& location.Test(v.Location, state)
				&& item.Test(v.Item, state);
	...
	public class BingoTriggerOnSabotageSetpiece : BingoTrigger {
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerLocation location;
		readonly BingoTriggerCIString RepositoryId = new();
		readonly BingoTriggerUInt EntityID = new();
		...
		public BingoTriggerOnSabotageSetpiece(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
			RepositoryId.Load(json, nameof(RepositoryId));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnSabotageSetpieceEventValue v
				&& base.Test(ev, state)
				&& player.Test(v.Player, state)
				&& location.Test(v.Location, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnShot : BingoTrigger {
		readonly BingoTriggerPlayer player;
		readonly BingoTriggerLocation location;
		readonly BingoTriggerCIString RepositoryId = new();
		readonly BingoTriggerUInt EntityID = new();
		...
		public BingoTriggerOnShot(JsonElement json) : base(json) {
			player = new(json);
			location = new(json);
			EntityID.Load(json, nameof(EntityID));
			RepositoryId.Load(json, nameof(RepositoryId));
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnShotEventValue v
				&& base.Test(ev, state)
				&& player.Test(v.Player, state)
				&& location.Test(v.Location, state)
				&& EntityID.Test(v.EntityID, state)
	...
	public class BingoTriggerOnTakeDamage(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is OnTakeDamageEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
	...
	public class BingoTriggerInstinctActive(JsonElement json) : BingoTrigger(json) {
		readonly BingoTriggerLocation location = new(json);
		readonly BingoTriggerPlayer trigger = new(json);

		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is InstinctActiveEventValue v
				&& base.Test(ev, state)
				&& trigger.Test(v.Player, state)
				&& location.Test(v.Location, state);
		}
	}
	...
	public class BingoTriggerDrainPipeClimbed(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is DrainPipeClimbedEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
	...
	public class BingoTriggerMovement(JsonElement json) : BingoTrigger(json) {
		...
		public override bool Test(EventValue ev, BingoTileState state) {
			return ev is MovementEventValue v
				&& base.Test(v, state)
				&& location.Test(v.Location, state)
				&& player.Test(v.Player, state);
		}
