...
- **Streaks**: consecutive SA completions will increase your streak counter which can optionally be displayed, along with the option of displaying your personal best, in the UI.

- **Rulesets**: basic support for rulesets provided in the mod. Extended support in the app. (See: App Features -> Rulesets)

- **Timer**: an RTA timer that automatically pauses between spins and during load screens, and can be integrated with LiveSplit.
...
- **Spin Bookmarking**: bookmark spins to remember and revisit them later.
...
- **In-Game Overlay**: for those with only one display available, a text-only in-game overlay will show the current spin conditions, time (optional) and current streak score (optional). This overlay can also be docked to one of the corners of the game window for minimal clutter.
